<?php
	...
	switch($cmd)
	{
		...
		case "login":
			$info["table"]     = "users";
			$info["fields"]   = array("*");
			$info["where"]    = " 1=1 AND email  LIKE BINARY '".clean($_REQUEST["email"])."' AND password  LIKE BINARY '".clean($_REQUEST["password"])."' AND user_type='super'";			
			$res  = $db->select($info);
			if(count($res)>0)
			{
		...
		case "forget_pass":
		      $info["table"]     = "users";
				...
				if(count($res)>0)
				{
					...
					$subject = "Recovery Password from correctleads";
					...
					//send email
						...
						$headers .= 'From: correctleads <info@correctleads.com>' . "\r\n";
